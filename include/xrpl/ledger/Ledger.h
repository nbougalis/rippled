...
public:
     ...
     * @return false if either map is Invalid, leaving the immutable flag unset. A
     *         map invalidated partway through can leave the header hashes written
     *         and one map immutable, so a false return means the ledger must be
     *         discarded rather than retried.
     */
    [[nodiscard]] bool
