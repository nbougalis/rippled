...
TransactionAcquire::takeNodesLocked(
    ...
    ScopedLockType&)
{
    // A reply that arrives after the outcome. trigger() sends to every peer it was given, so once
    // one of them settles the set the others' replies are already in flight, and neither they nor
    // the peer whose data failed the set could have known. Those are solicited, and free: one per
    // peer we asked, which is what bounds the honest case.
    //
    // Past that the sender is replaying, and this is not free work to serve.
    // InboundTransactions::gotData() deserializes and hashes the whole node list before this point,
    // up to kHardMaxReplyNodes of them, and the object lingers until newRound() sweeps it, so an
    // unbounded number of replays would otherwise cost only the trivial per-message fee.
    if (isDone())
    {
...
TransactionAcquire::stillNeed()
{
    ...
    // that just failed. Reviving starts a new round of asks, so a stale count must not carry over
    // and eat into it, mischarging a reply the new round is still owed.
    lateReplies_ = 0;
